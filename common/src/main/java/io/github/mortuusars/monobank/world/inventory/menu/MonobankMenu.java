...
public class MonobankMenu extends AbstractContainerMenu {
    ...
    public MonobankMenu(int containerID, Inventory playerInventory, MonobankBlockEntity blockEntity, MonobankExtraInfo extraInfo) {
        ...
        // Monobank slot
        addSlot(new BigSlot(blockEntity, 0, MONOBANK_SLOT_X, MONOBANK_SLOT_Y,
    ...
    public void clicked(int slotId, int button, ClickType clickType, Player player) {
        ...
        if (button == TRANSFER_SINGLE_BUTTON_ID) {
            ItemStack item = blockEntity.removeItemNoUpdate(0);
            ...
            if (!item.isEmpty()) {
                blockEntity.setItem(0, item); // Insert remainder back
            }
        }
        else {
            super.clicked(slotId, button, clickType, player);
    ...
    public @NotNull ItemStack quickMoveStack(Player pPlayer, int index) {
        ...
        if (index == MONOBANK_SLOT_INDEX) { // From Monobank
            ...
            Slot monobankSlot = this.slots.getFirst();
            if (!monobankSlot.hasItem() || ItemStack.isSameItemSameComponents(monobankSlot.getItem(), clickedSlotItemStack)) {
